    ...
    <div className="grid grid-cols-1 selection:text-white gap-3">
        {SearchResults.collection.items.map((data)=>(
            <div key={data.title} className="flex flex-col justify-center items-center">
                ...
                {data.data.map((data)=>(
                <div key={data.title} className="order-2 group-hover:text-blue-500 text-slate-700">
                <p className="text-[10px] text-slate-500">{data.date_created}</p>
                <Link href={`/detail/${data.title}`}><h1 className=" line-clamp-1 hover:underline w-full">{data.title}</h1></Link>
                <p className="text-xs text-slate-400 line-clamp-2">{data.description}</p>
                </div>
                ))}
            {data.links.map((data)=>(
            <div key={data.render} className=" h-full overflow-hidden select-none order-1 flex items-center border-l-4 group-hover:border-zinc-400 group-hover:scale-110">
