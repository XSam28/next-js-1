...
const pathname = usePathname()
  return (
    <>
    <div onClick={()=>props.clos()} className="lg:hidden w-[40vw] h-[100vh] z-[9999] fixed bg-black/80 top-0 left-0"></div>
    <div className="w-[60vw] h-[100vh] z-[999999] fixed bg-black top-0 right-0 lg:hidden">
    ...
    <h1 className="text-2xl">SPACE.BLOG</h1>
        ...
        <div className="flex justify-center items-center w-[60vw] h-[90vh] flex-col gap-5">
          {link.map((link)=>
        <Link onClick={()=>props.clos()} className={`text-white hover:text-black duration-75 hover:bg-white p-3 w-full text-center ${pathname === link.url ? "underline" : ""}`} key={link.id} href={link.url}>{link.title}</Link>
        )}
        </div>
